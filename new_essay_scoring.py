...
def scoring(bert_model, gru_model, extracted_features, tokenizer):
	...
	scaler = pd.read_csv(
		"./features/scaler.csv", encoding="cp949"
	)
...
def get_qwk_accuracy(gru_model, bert_model, tokenizer):
	essays = os.listdir('./valid_json')
	valid_dataset = pd.read_csv('./feat_valid.csv',encoding='utf-8-sig')
	
	pred_list = []
	real_list = []
	for essay in tqdm(essays[:]):
		with open("./valid_json/" + essay, "r", encoding="utf-8-sig") as f:
			extracted_features = json.load(f)
		f.close()
		output, _ = scoring(bert_model, gru_model, extracted_features, tokenizer)
		real_score = valid_dataset[valid_dataset['essay_id'] == essay.replace('.json','')].iloc[0]['essay_score_avg']
		real_score_list =  np.rint([float(st) for st in real_score.split('#')]).astype(int)
		pred_list.append(output)
		real_list.append(real_score_list)
	
	pred_list = np.array(pred_list)
	real_list = np.array(real_list)

	accuracy_scores = np.mean(pred_list == real_list, axis=0)
	kappa_scores = [cohen_kappa_score(pred_list[:,i], real_list[:,i], weights='quadratic') for i in range(pred_list.shape[1])]
	# qwk_scores = []
	# acc_scores = []
	# for i in range(real_list.shape[1]):
	# 	real_col = real_list[:,i]
	# 	pred_col = pred_list[:,i]
	# 	qwk = cohen_kappa_score(real_col, pred_col, weights='quadratic')
	# 	qwk_scores.append(qwk)

	# 	acc = accuracy_score(real_col,pred_col)
	# 	acc_scores.append(acc)

	return kappa_scores, accuracy_scores

if __name__ == "__main__":

	# extracted_features = process(text)
	with open("./valid_json/ESSAY_32680.json", "r", encoding="utf-8-sig") as f:
		extracted_features = json.load(f)
	f.close()
	...
	# scoring 모델
	gru_model = GRUScoreModule().to(device)
	gru_model.load_state_dict(torch.load("./model/kobert_model_attention4.pth", weights_only=True))

	qwk, acc = get_qwk_accuracy(gru_model,bert_model,tokenizer)
	print(qwk)
	print(acc)
	# 글 점수 및 top-k 자질 출력
	# output, top_k_features = scoring(bert_model, gru_model, extracted_features, tokenizer)
